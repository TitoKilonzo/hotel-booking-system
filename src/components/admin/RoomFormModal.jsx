    ...
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Room' : 'Add Room'} className="max-w-xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Select label="Room Type *" value={form.type} onChange={set('type')} error={errors.type}>
            {ROOM_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </Select>
          <Input label="Price / Night ($) *" type="number" value={form.pricePerNight} onChange={set('pricePerNight')} error={errors.pricePerNight} min={1} />
        </div>

        <div className="grid grid-cols-3 gap-3">
          <Input label="Capacity *" type="number" value={form.capacity} onChange={set('capacity')} error={errors.capacity} min={1} max={20} />
          <Input label="Size (m²)" type="number" value={form.size} onChange={set('size')} min={1} />
          <Input label="Floor" type="number" value={form.floor} onChange={set('floor')} min={0} />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <Input label="Room Number" value={form.roomNumber} onChange={set('roomNumber')} placeholder="101" />
        ...
        <Textarea label="Description" value={form.description} onChange={set('description')}
          placeholder="Room details, view, included amenities..." rows={2} />
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button type="submit" className="flex-1" loading={saving}>{isEdit ? 'Save Changes' : 'Add Room'}</Button>
        </div>
      </form>
    </Modal>
  )
